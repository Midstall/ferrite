...
const VIRTIO_RNG_DEV_ID: u16 = 0x1044; // 0x1040 + subsystem 4
const VIRTIO_LEGACY_DEV_ID: u16 = 0x1005; // transitional, subsystem 4
const VIRTIO_RNG_SUBSYSTEM: u16 = 4;
...
var avail: *volatile AvailRing = undefined;
var used: *volatile UsedRing = undefined;
var bufs_va: usize = 0;
var bufs_pa: u64 = 0;
...
// level-triggered line), the device's GIC INTID, the IRQ handle, and the
// IRQ-wait channel recv handle. irq_num == 0 means no usable IRQ.
...
pub fn main() void {
    ...
    fs.registerDevice("random", .char, svc_send) catch |e| {
        ferrite.console.print("[virtio-rng] registerDevice failed: {t}\n", .{e}) catch {};
        return;
    };
...
fn findAndInit() bool {
    ...
    if (!findDevice(&bdf_buf)) {
        ferrite.console.print("[virtio-rng] no device on PCI\n", .{}) catch {};
        return false;
    }
    ...
    const wanted = VIRTIO_F_VERSION_1;
    if ((dev_features & wanted) != wanted) {
        ferrite.console.print("[virtio-rng] device lacks VERSION_1 (features=0x{x})\n", .{dev_features}) catch {};
    ...
    // IRQ disabled: inline block-on-IRQ in drawRandom is incompatible with
    // shared, level-triggered INTx (an idle rng in fs.serve never acks the
...
fn setupQueue() bool {
    w16(common_base + C.QUEUE_SELECT, VQ_REQUEST);
    ...
    desc = @ptrFromInt(@as(usize, @intCast(ring_va)));
    avail = @ptrFromInt(@as(usize, @intCast(ring_va)) + DESC_BYTES);
    used = @ptrFromInt(@as(usize, @intCast(ring_va)) + USED_OFFSET);
    bufs_va = @intCast(b_va);
    bufs_pa = b_pa;
    last_used = 0;

    var i: u32 = 0;
    while (i < QSIZE) : (i += 1) {
        desc[i] = .{ .addr = 0, .len = 0, .flags = 0, .next = 0 };
    }
    avail.* = std.mem.zeroes(AvailRing);
    ...
    notify_addr = notify_base + (@as(usize, notify_off) * @as(usize, notify_multiplier));

    w16(common_base + C.QUEUE_ENABLE, 1);
    return true;
}

fn primeRequests() void {
    var i: u16 = 0;
    while (i < QSIZE) : (i += 1) {
        desc[i] = .{
            .addr = bufs_pa + @as(u64, i) * BUF_SIZE,
            .len = BUF_SIZE,
            .flags = DESC_WRITE,
            .next = 0,
        };
...
fn onRead(s: *State, fid: u32, _: u64, out: []u8) fs.HandlerError!usize {
    ...
    return drawRandom(out);
}

// Wait until the device fills at least one entropy buffer (used.idx advances).
// IRQ-driven when irq_num != 0: block on the INTx channel, read the ISR + ack
// on every wake (after re-checking used.idx). On recv error, fall through to
// the bounded poll so a missed IRQ can't hang a read forever. Returns true once
// entropy is ready, false on the deadline timeout.
fn waitForEntropy() bool {
    ...
    if (irq_num != 0) {
        ...
        while (used.idx == last_used) {
            ...
            // On every wake: read the ISR (de-asserts the shared, level-triggered
            // line) and ack (unmasks the GIC line), then re-check used.idx.
            if (isr_base != 0) _ = @as(*volatile u8, @ptrFromInt(isr_base)).*;
            _ = ferrite.irqAck(irq_handle);
            if (used.idx != last_used) break;
            if (n < 0) return spinForEntropy(deadline); // recv failed: bounded poll
            if (ferrite.clockMono() > deadline) return false;
        }
        return true;
    }

    return spinForEntropy(deadline);
}

// Bounded yield/deadline poll. Used for no-IRQ arches and as the missed-IRQ
// fallback. Returns true once entropy is ready, false on timeout.
fn spinForEntropy(deadline: u64) bool {
    while (used.idx == last_used) {
        if (ferrite.clockMono() > deadline) return false;
        ferrite.yield();
    }
...
fn drawRandom(out: []u8) usize {
    if (out.len == 0) return 0;
    if (!waitForEntropy()) return 0;
    ferrite.barrier.full();

    const slot = last_used % QSIZE;
    const u = used.ring[slot];
    last_used +%= 1;

    const desc_idx: u16 = @intCast(u.id);
    const got: usize = @intCast(u.len);
    const src_va = bufs_va + @as(usize, desc_idx) * BUF_SIZE;
    const src_p: [*]const u8 = @ptrFromInt(src_va);
    const n = @min(got, out.len);
    @memcpy(out[0..n], src_p[0..n]);

    avail.ring[avail.idx % QSIZE] = desc_idx;
    ferrite.barrier.storeStore();
    avail.idx +%= 1;
    ferrite.barrier.full();
    w16(notify_addr, VQ_REQUEST);
...
fn matchDevice(bdf: [12]u8) bool {
    ...
    return dev_id == VIRTIO_RNG_DEV_ID or dev_id == VIRTIO_LEGACY_DEV_ID;
}
