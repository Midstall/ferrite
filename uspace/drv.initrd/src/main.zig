...
const URI_PREFIX = "com.midstall.ferrite.drv.initrd@v0";
const DEV_NAME = "initrd";
...
pub fn main() void {
    const total = ferrite.initrdSize();
    if (total <= 0) {
        ferrite.console.print("[drv.initrd] no initrd in kernel. Exiting\n", .{}) catch {};
        return;
    }
    ...
    if (ch < 0) {
        ferrite.console.print("[drv.initrd] channelCreate failed: {d}\n", .{ch}) catch {};
        return;
    }
    const svc_send: u32 = @truncate(@as(u64, @bitCast(ch)));
    const svc_recv: u32 = @truncate(@as(u64, @bitCast(ch)) >> 32);

    fs.registerDevice(DEV_NAME, .block, svc_send) catch |e| {
        ferrite.console.print("[drv.initrd] registerDevice failed: {t}\n", .{e}) catch {};
        return;
    };
...
fn onRead(s: *State, fid: u32, offset: u64, out: []u8) fs.HandlerError!usize {
    ...
    const n = ferrite.initrdRead(offset, out);
